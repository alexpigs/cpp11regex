...
class CPlayer {
	...
	public:
		static CPlayer *getInstance() {
			static CPlayer *s_player = NULL;
			if (s_player == NULL){
				s_player = new CPlayer();
			}
			return s_player;
		}
		void play(const std::string& note, float gain){
		}
};
...
class CBaseNote {
	...
	private:
		eNoteType m_type;
		std::vector<std::string> m_vecNotes;
		int m_nPlayTime;
		void DoDestroy(){
			m_vecNotes.clear();
		}
	...
	public:
		...
		void play(int bpm, float rate, float gain){
			if (m_type == Simple){
				PLAYER()->play(m_vecNotes[0], gain); 
			}else if (m_type == Chord){
				for(auto note : m_vecNotes){
					PLAYER()->play(note, gain);
				}
			}else if (m_type == Arpeggio){
				float time = 60.0f / bpm * m_nPlayTime * rate / 32.0f;
			}
		}
		...
		friend std::ostream & operator <<(std::ostream& os, const CBaseNote& t1){
			os << "CBaseNote:" << t1.m_nPlayTime  << " notes:" <<t1.m_vecNotes.size() << std::endl;
...
class CMusicNote {
	...
	public:
		...
		void doPlay(CBaseNote* note, int bpm, float ratio, float gain){
			note->play(bpm, ratio, gain);
		}

		// return true if not finish, false means finish
		bool play(int bpm, float ratio, bool playBacking) {
			ratio = 60.0f / bpm * ratio / 32.0f;
			int trebleTimes = 0;
			for(auto treble : m_TrebleNotes){
				TIMER()->ScheduleOnce((int)(trebleTimes * ratio), std::bind(&CMusicNote::doPlay, this, treble, bpm, ratio, 0.3f));
				trebleTimes += treble->getPlayTime();
			}

			if (playBacking){
				int backingTimes = m_nBackingOffsetTimes;
				for(auto backing : m_BackingNotes){
					TIMER()->ScheduleOnce((int)(backingTimes * ratio), std::bind(&CMusicNote::doPlay, this, backing, bpm, ratio, 0.3f));
					backingTimes += backing->getPlayTime();
				}
			}
		}
...
class CSongParser {
	...
	public:
		...
		CBaseNote::eNoteType getNoteType(const std::string& type){
			static struct {
				char mark;
				CBaseNote::eNoteType type;
			} s_NoteType[] = {
				{'.', CBaseNote::eNoteType::Chord},
				{'~', CBaseNote::eNoteType::Legato},
				{'@', CBaseNote::eNoteType::Arpeggio},
				{'&', CBaseNote::eNoteType::Wave},
				{'^', CBaseNote::eNoteType::Trill},
				{'$', CBaseNote::eNoteType::Echo},
				{'%', CBaseNote::eNoteType::LAppoggiatura},
				{'!', CBaseNote::eNoteType::SAppoggiatura},
				{0, CBaseNote::eNoteType::Simple}
			};
			CBaseNote::eNoteType ret = CBaseNote::eNoteType::Simple;
			if (type.size() != 1){
				return ret;
			}

			int i = 0;
			while(s_NoteType[i].mark != 0){
				if (s_NoteType[i].mark == type.at(0)){
					ret = s_NoteType[i].type;
					break;
				}
				++i;
			}
		...
		CBaseNote* parse_combine_note(const std::string& note){
			...
			try{
				...
				if (std::regex_match(s,m,re_note_cmb)){
					...
					if (tmp.size() > 0){
						CBaseNote* note = new CBaseNote(getNoteType(flag), time);
						for(auto str : tmp)
							note->addNote(str);
						return note;
					}
